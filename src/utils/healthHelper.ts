 ...
 *
 * 这套阈值原先散在 Node.tsx 的内联三元与 MiniBars.tsx 的 colorOf 里各写一遍;
 * 统一收到这里, 改档位只改一个文件。
 *
 * 只管"档位与颜色", 不管底纹长度: 这两格底下的 MiniBars 是逐点着色的,
 * 再叠一层按聚合值涂满的底纹会把逐点信息糊掉 (延迟看不出偶发尖峰, 丢包
 * 看不出丢在哪一段), 所以健康格不画底纹, 只用数值的阈值色。
 */
